import { softPanel, textPrimary, textSecondary, textTertiary } from "@/shared/ui/tailwind";
        ...
        <section className="grid gap-4 py-7 sm:py-10 sm:pb-6">
          ...
          <div className="grid items-end gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(260px,0.8fr)]">
            <div>
              ...
              <p className={`mt-4 mb-0 max-w-[40rem] text-base leading-[1.7] ${textSecondary}`}>
            ...
            <dl className="m-0 grid gap-3" aria-label="서비스 핵심 정보">
              <div className={`${softPanel} rounded-[20px] px-4 py-[1.05rem]`}>
                <dt className={`text-[0.82rem] font-semibold ${textTertiary}`}>세션 기반</dt>
                <dd className={`mt-[0.35rem] ml-0 leading-6 ${textPrimary}`}>
                  기록 없이 지금 이 화면에서만 생성
                </dd>
              </div>
              <div className={`${softPanel} rounded-[20px] px-4 py-[1.05rem]`}>
                <dt className={`text-[0.82rem] font-semibold ${textTertiary}`}>3분 길이</dt>
                <dd className={`mt-[0.35rem] ml-0 leading-6 ${textPrimary}`}>
                  짧고 반복 가능한 개인 청취 리추얼
                </dd>
              </div>
